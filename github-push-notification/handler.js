...
module.exports.handler = (event, context, callback) => {
    ...
    var githubHmac = event.headers['X-Hub-Signature'].replace('sha1=','');
    var lambdaHmac = crypto.createHmac('sha1',config.github.secret).update(JSON.stringify(event.body)).digest('hex');

    if(githubHmac !== lambdaHmac){
        context.fail("Invalid GitHub Secret",null);
    } else { 

        var commits = event.body.commits;
        ...
        commits.forEach(function(e,i,a){
            ...
            .then((diff)=>{
                ...
                email += 'Branch: ' + event.body.ref + '<br/>';
                email += 'Home: <a href="' + event.body.repository.html_url + '">' + event.body.repository.html_url + '</a><br/>';
                email += 'Commit: <a href="' + e.url + '">' + e.id + '</a><br/>';
                email += 'Author: <a href="mailto:' + e.author.email + '">' + e.author.name + '</a><br/>';
                email += 'Compare: <a href="' + event.body.compare + '">' + event.body.compare + '</a><br/>';
            ...
            .then((c)=>{
                var subject = '[' + event.body.repository.full_name + '] ' + c.commit.id.substring(0,6) + ': ' + c.commit.message;
